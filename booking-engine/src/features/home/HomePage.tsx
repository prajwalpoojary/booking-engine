...
// Define the property type based on the properties data
interface Property {
  id: number;
  name: string;
  location: string;
...
function useHotelFilter() {
  ...
  const [maxPrice, setMaxPrice] = useState<string | 'All'>('All');
  ...
  const filteredProperties = properties.filter(p => {
    ...
    const matchesPrice = maxPrice === 'All' || (typeof maxPrice === 'number' && minPrice <= maxPrice);
    return matchesPrice;
  });
...
const RecentlyViewed = () => {
  ...
  const handleClick = (hotel: Property) => {
    addHotel(hotel);
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <select
              value={maxPrice}
              onChange={(e) => setMaxPrice(e.target.value as string | 'All')}
